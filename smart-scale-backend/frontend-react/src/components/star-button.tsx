...
import { useRef, useEffect, type ReactNode, type CSSProperties, type ButtonHTMLAttributes } from "react"
...
export function StarButton({
  ...
  const themeBg = backgroundColor || (isGreen ? "#22c55e" : "#667eea")
  const textGradient = isGreen
    ? "from-white to-green-100"
    : "from-white to-purple-100"

  useEffect(() => {
    // 注入动画 keyframes（仅一次）
    const styleId = "star-btn-keyframes"
    if (!document.getElementById(styleId)) {
      const style = document.createElement("style")
      style.id = styleId
      style.textContent = `
        @keyframes star-btn-move {
          0% { offset-distance: 0%; }
          100% { offset-distance: 100%; }
        }
        .animate-star-btn {
          animation: star-btn-move var(--duration, 3s) linear infinite;
        }
      `
  ...
  return (
    <button
      ...
      ref={pathRef}
      className={cn(
        "relative z-[3] overflow-hidden h-10 px-5 py-2 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-3xl text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 group/star-button",
        className
      )}
      {...props}
    >
      ...
      <div
        ...
        aria-hidden="true"
      >
        <StarBackground color={themeBg} />
      </div>
      <span className={cn("z-10 relative bg-gradient-to-t inline-block text-transparent bg-clip-text font-semibold", textGradient)}>
        {children}
      </span>
    </button>
  )
