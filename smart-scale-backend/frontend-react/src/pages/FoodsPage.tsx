      ...
      <div className="w-full max-w-2xl max-h-[90vh] lg:max-h-none overflow-y-auto" style={{ perspective: "1000px" }} onClick={e => e.stopPropagation()}>
        <div ref={cardRef} onMouseMove={onMove} onMouseLeave={onLeave}
          ...
          <div style={{ transform: "translateZ(60px)" }} className="relative z-10 mt-2 flex flex-wrap items-center justify-center gap-3">
            <h2 className="text-3xl font-bold tracking-tight text-gray-900">{food.name}</h2>
            <LiquidGlassButton color={c.from} className="!px-4 !py-1.5 !text-sm">
              {emj} {food.category || "其他"}
            </LiquidGlassButton>
          </div>

          <div style={{ transform: "translateZ(40px)" }} className="relative z-10 mt-2 text-center">
            <p className="text-sm text-gray-500">{food.name_en}</p>
            {food.edible_ratio ? (
              <p className="mt-1 text-xs">可食部占比: <b style={{ color: c.text }}>{(food.edible_ratio * 100).toFixed(0)}%</b></p>
            ) : null}
          </div>
            ...
            {[["🔥","能量",food.energy_kcal,"kcal"],["💪","蛋白质",food.protein_g,"g"],["🧈","脂肪",food.fat_g,"g"],["🍚","碳水",food.carbohydrate_g,"g"]]
              .map(([icon, label, value, unit]) => (
                <div key={String(label)} className="rounded-xl px-2 py-3 text-center shadow-sm" style={{ backgroundColor: c.bg }}>
                  ...
                  <div className="text-lg font-extrabold" style={{ color: c.text }}>{metric(value as number)}</div>
          ...
          {detailFields.some(([k]) => (food[k] as number) > 0) && (
            <div className="relative z-10 mt-5 pt-4" style={{ transform: "translateZ(20px)" }}>
              <h5 className="mb-2 text-sm font-semibold text-gray-700">详细营养数据（每100g）</h5>
              <div className="overflow-hidden rounded-xl border" style={{ borderColor: c.from }}>
                <table className="min-w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100 bg-white">
                    {detailFields.filter(([k]) => (food[k] as number) > 0).map(([key, label, unit, icon]) => (
                      <tr key={key}>
                        <td className="px-4 py-2 text-gray-700">{icon} {label}</td>
                        <td className="px-4 py-2 font-medium" style={{ color: c.text }}>{metric(food[key])} {unit}</td>
                      </tr>))}
                  </tbody>
                </table>
              </div>
              ...
              {paged.map((food, idx) => {
                ...
                return (
                  <motion.div key={`${food.id}-${ak}`}
                    ...
                    <Card
                      className="cursor-pointer border border-transparent bg-white/85 transition-all hover:-translate-y-1"
                          ...
                          {[["🔥","能量",metric(food.energy_kcal),"kcal"],["💪","蛋白质",metric(food.protein_g),"g"],["🧈","脂肪",metric(food.fat_g),"g"],["🍚","碳水",metric(food.carbohydrate_g),"g"]]
                            .map(([icon, l, value, unit]) => (
                              <div key={String(l)} className="rounded-lg px-2 py-1.5" style={{ backgroundColor: c.bg }}>
                                <div className="mb-0.5 text-xs">{icon}</div>
                                <div className="text-[10px] text-gray-500">{l}</div>
                                <div className="text-sm font-bold" style={{ color: c.text }}>{value}<span className="ml-0.5 text-[10px] font-normal text-gray-400">{unit}</span></div>
                              </div>))}
                        </div>
                      </CardContent>
                    </Card>
          ...
          {totalPages > 1 && (
            <div className="mt-6 flex items-center justify-center gap-2">
              <Button variant="outline" size="sm" disabled={page <= 1} onClick={() => { setPage(p => p - 1); setAk(k => k + 1) }}
                className="border-green-200 text-green-700 hover:bg-green-50 disabled:opacity-40"><ChevronLeft className="h-4 w-4" />上一页</Button>
              ...
              <Button variant="outline" size="sm" disabled={page >= totalPages} onClick={() => { setPage(p => p + 1); setAk(k => k + 1) }}
                className="border-green-200 text-green-700 hover:bg-green-50 disabled:opacity-40">下一页<ChevronRight className="h-4 w-4" /></Button>
            </div>
          )}
          <p className="mt-2 text-center text-xs text-gray-400">第 {page} / {totalPages} 页，共 {visible.length} 种食物</p>
        </>
      )}
