...
import { FileText, Clock, Trash2, Zap, ChevronLeft, ChevronRight, Trash, X, ChevronsLeft, ChevronsRight, Info, Apple, Sparkles } from "lucide-react"
import { WaveLoader } from "@/components/wave-loader"
import { motion, AnimatePresence } from "framer-motion"

// 手机端每页 6 条，桌面端 10 条（模块加载时按屏宽决定一次）
const PAGE_SIZE = typeof window !== "undefined" && window.innerWidth < 1024 ? 6 : 10
...
function ReportDetailModal({ summary, onClose }: { summary: AnalysisSummary; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/45 px-4 py-8" onClick={onClose}>
      {/* 外层 max-h 不再被内部滚动，内部 flex-col 撑满，让头部固定 */}
      <div className="w-full max-w-3xl max-h-[85vh] flex flex-col rounded-3xl bg-white border border-gray-200 shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="relative flex items-center justify-between border-b px-3 lg:px-6 py-2 lg:py-4 shrink-0" style={{ backgroundColor: tg.bg }}>
          <div>
          ...
          <button onClick={onClose} className="rounded-full p-1.5 lg:p-2 text-gray-400 hover:bg-gray-100"><X className="h-4 w-4 lg:h-5 lg:w-5" /></button>
        ...
        {!isDaily && (
          <div className="px-3 lg:px-6 pt-3 lg:pt-4 shrink-0">
            <div className="flex gap-1.5 lg:gap-2 bg-gray-100 p-1 rounded-xl">
              {TAB_ITEMS.map(tab => {
                const Icon = tab.icon
                return (
                  <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                    className="relative px-2 py-1 lg:px-4 lg:py-2 text-[11px] lg:text-sm font-medium rounded-lg text-gray-600 hover:text-gray-900">
                    ...
                    <span className="relative z-10 flex items-center gap-1 lg:gap-1.5"><Icon className="h-3.5 w-3.5 lg:h-4 lg:w-4" />{tab.label}</span>
                  </button>
                )
              })}
            </div>
        ...
        {/* 内容区：单独滚动，flex-1 撑满剩余高度 */}
        <div ref={scrollRef} onScroll={updateScrollProgress} className="p-2.5 lg:p-6 min-h-0 flex-1 overflow-y-auto hide-scrollbar relative">
              ...
              <motion.div key="basic" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }} transition={{ duration: 0.25, ease: "easeOut" }}>
                {basicGroups.length === 0 ? <p className="text-gray-400 text-center py-8">暂无基本数据</p> : (
                  <div className="space-y-3 lg:space-y-4 overflow-hidden">
                    {basicGroups.map((group, gi) => {
                      ...
                      return (
                        <div key={gi}>
                          <h6 className="text-[10px] lg:text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5 lg:mb-2">{group.label}</h6>
                          <div className="grid gap-1.5 lg:gap-2 sm:grid-cols-2">
                            {group.items.map((item, ii) => (
                              <motion.div key={ii}
                                ...
                                className={`flex items-center justify-between rounded-lg lg:rounded-xl px-2.5 py-1.5 lg:px-3 lg:py-2.5 shadow-sm ${cardBg}`}>
                                <span className="text-[11px] lg:text-xs text-gray-500 font-medium">{item.label}</span>
                                <span className="text-[11px] lg:text-xs font-bold flex items-baseline" style={item.color ? { color: item.color } : undefined}>
                                  {isNum(item.value) ? (
                                    <AnimatedNumber value={Number(item.value)} duration={0.6} decimals={1} />
                                  ) : item.value}
                                  {item.unit ? <span className="ml-0.5 text-[9px] lg:text-[10px] text-gray-400 font-normal">{item.unit}</span> : null}
                                </span>
                              </motion.div>
                            ))}
                          </div>
                ...
                {isDaily && topFoods.length > 0 && (
                  <div className="mt-3 lg:mt-4">
                    <h6 className="text-[10px] lg:text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5 lg:mb-2">食物统计</h6>
                    <div className="flex flex-wrap gap-1.5 lg:gap-2">
                      {topFoods.map((f, i) => (
                        <span key={`${f.name || f.name_en}-${i}`}
                          className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 lg:px-3 lg:py-1 text-[11px] lg:text-xs font-medium border"
                ...
                {topFoods.length === 0 ? <p className="text-gray-400 text-center py-8">暂无数据</p> : (
                  <div className="space-y-2 lg:space-y-3">
                    {topFoods.slice(0, 10).map((f, i) => (
                      <div key={`${f.name || f.name_en}-${i}`} className="flex items-center gap-2 lg:gap-3">
                        <span className="w-5 lg:w-6 text-xs lg:text-sm font-bold text-gray-400 text-center">{i + 1}</span>
                        <div className="flex-1">
                          <div className="flex justify-between text-xs lg:text-sm mb-0.5 lg:mb-1"><span className="font-medium">{f.name || f.name_en}</span><span className="text-gray-400 text-[11px] lg:text-xs">{f.count}次</span></div>
                        ...
                        {f.total_weight_g ? <span className="text-[11px] lg:text-xs text-gray-400 w-10 lg:w-12 text-right">{Math.round(f.total_weight_g)}g</span> : null}
                      </div>
                    ))}
                  </div>
                )}
                ...
                {(() => {
                  ...
                  return (
                    <div className="space-y-3 lg:space-y-4">
                      {aiSummary && (
                        <div className="rounded-xl lg:rounded-2xl p-3 lg:p-4 border" style={{ backgroundColor: tg.bg, borderColor: `${tg.from}40` }}>
                          <div className="flex items-center gap-1.5 lg:gap-2 mb-1.5 lg:mb-2">
                            <Sparkles className="h-3.5 w-3.5 lg:h-4 lg:w-4" style={{ color: tg.to }} />
                            <h6 className="text-xs lg:text-sm font-semibold" style={{ color: tg.to }}>本期总结</h6>
                          </div>
                          <p className="text-xs lg:text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{aiSummary}</p>
                        </div>
                      )}
                      {aiAdvice && (
                        <div className="rounded-xl lg:rounded-2xl p-3 lg:p-4 border bg-white" style={{ borderColor: `${tg.from}40` }}>
                          <div className="flex items-center gap-1.5 lg:gap-2 mb-1.5 lg:mb-2">
                            <Zap className="h-3.5 w-3.5 lg:h-4 lg:w-4" style={{ color: tg.to }} />
                            <h6 className="text-xs lg:text-sm font-semibold" style={{ color: tg.to }}>下期建议</h6>
                          </div>
                          <p className="text-xs lg:text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{aiAdvice}</p>
                        </div>
                      )}
                    </div>
                  )})()}
...
export default function ReportsPage() {
  ...
  return (
    <AppShell title="营养分析报告" titleIcon={<FileText className="w-6 h-6 text-green-600" />} theme="green">
      {feedback && (
        <div className="mb-2 lg:mb-4 rounded-xl bg-emerald-50 border border-emerald-200 px-3 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm text-emerald-700">{feedback}</div>
      )}
      ...
      {loading ? (
        ...
        <>
          {/* 双列网格 + 依次出现动画 */}
          <div className="grid gap-1.5 lg:gap-3 grid-cols-1 lg:grid-cols-2">
            <AnimatePresence mode="wait">
              {summaries.map((s, idx) => {
                ...
                return (
                  ...
                  >
                    <Card
                      className="cursor-pointer border-0 hover:-translate-y-0.5 hover:shadow-lg transition-all relative group py-2 lg:py-6 gap-2 lg:gap-6"
                      style={{ backgroundColor: tg.bg }}
                      onClick={() => setSelected(s)}>
                      <CardContent className="p-1.5 lg:p-2.5">
                        {/* 左右分区 */}
                        <div className="flex gap-2 lg:gap-3">
                          {/* 左侧：标签 + 日期 */}
                          <div className="flex-shrink-0 flex flex-col items-start min-w-0" style={{ minWidth: "38%" }}>
                            <span className="inline-flex items-center gap-1 rounded-full px-2 lg:px-2.5 py-0 lg:py-0.5 text-[10px] lg:text-[11px] font-bold text-white whitespace-nowrap"
                              style={{ backgroundImage: `linear-gradient(to right, ${tg.from}, ${tg.to})` }}>
                              {tg.emoji} {tg.label}
                            </span>
                          ...
                          <div className="flex-1 min-w-0">
                            {i && (
                              <div className="grid grid-cols-2 gap-x-1 lg:gap-x-2 gap-y-0.5">
                                {i.total_energy_kcal != null && (
                                  <span className="text-[10px] lg:text-xs text-gray-600">🔥 {Math.round(i.total_energy_kcal)} kcal</span>
                                )}
                                {i.total_protein_g != null && (
                                  <span className="text-[10px] lg:text-xs text-gray-600">💪 {rv(i.total_protein_g)}g</span>
                                )}
                                {i.total_fat_g != null && (
                                  <span className="text-[10px] lg:text-xs text-gray-600">🧈 {rv(i.total_fat_g)}g</span>
                                )}
                                {i.total_carbohydrate_g != null && (
                                  <span className="text-[10px] lg:text-xs text-gray-600">🍚 {rv(i.total_carbohydrate_g)}g</span>
                                )}
                              </div>
                            )}
                            {i?.top_foods?.length ? (
                              <p className="text-[9px] lg:text-[11px] text-gray-400 mt-1 lg:mt-1.5 truncate">
                        ...
                        <button type="button" disabled={deletingId === s.id}
                          ...
                          className="absolute top-1.5 right-1.5 lg:top-2 lg:right-2 rounded-full p-1 lg:p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 transition-all opacity-100 lg:opacity-0 lg:group-hover:opacity-100"
                          title="删除"><Trash2 className="h-3 lg:h-3.5 w-3 lg:w-3.5" /></button>

                        {/* AI总结标记 — 有ai_summary时显示，右下角 */}
                        {i?.ai_summary && (
                          <span className="absolute bottom-1.5 right-1.5 lg:bottom-2 lg:right-2 inline-flex items-center gap-0.5 rounded-full px-1.5 py-0 lg:py-0.5 text-[9px] lg:text-[10px] font-bold text-white"
                            style={{ backgroundImage: `linear-gradient(to right, ${tg.from}, ${tg.to})` }}
                            title="已生成AI总结">
          ...
          {/* 分页 */}
          {totalPages > 1 && (
            <div className="mt-3 lg:mt-6 flex items-center justify-center gap-0.5 lg:gap-1 flex-wrap">
              <Button variant="outline" size="sm" disabled={page <= 1} onClick={() => goPage(1)}
                className="border-emerald-200 text-emerald-700 hover:bg-emerald-50 disabled:opacity-40 h-7 px-2"><ChevronsLeft className="h-3.5 w-3.5 lg:h-4 lg:w-4" /></Button>
              <Button variant="outline" size="sm" disabled={page <= 1} onClick={() => goPage(page - 1)}
                className="border-emerald-200 text-emerald-700 hover:bg-emerald-50 disabled:opacity-40 h-7 px-2"><ChevronLeft className="h-3.5 w-3.5 lg:h-4 lg:w-4" /></Button>
              ...
              {(() => {
                ...
                return Array.from({ length: end - start + 1 }, (_, i) => start + i).map(p => (
                  <button key={p} onClick={() => goPage(p)}
                    className={`h-6 w-6 lg:h-8 lg:w-8 rounded-full text-[11px] lg:text-sm font-medium transition ${p === page ? "bg-gradient-to-r from-emerald-500 to-green-600 text-white" : "text-gray-500 hover:bg-gray-100"}`}>{p}</button>
                ))
              })()}

              <Button variant="outline" size="sm" disabled={page >= totalPages} onClick={() => goPage(page + 1)}
                className="border-emerald-200 text-emerald-700 hover:bg-emerald-50 disabled:opacity-40 h-7 px-2"><ChevronRight className="h-3.5 w-3.5 lg:h-4 lg:w-4" /></Button>
              <Button variant="outline" size="sm" disabled={page >= totalPages} onClick={() => goPage(totalPages)}
                className="border-emerald-200 text-emerald-700 hover:bg-emerald-50 disabled:opacity-40 h-7 px-2"><ChevronsRight className="h-3.5 w-3.5 lg:h-4 lg:w-4" /></Button>
            </div>
          )}
          <p className="mt-1.5 lg:mt-2 text-center text-[10px] lg:text-xs text-gray-400">第 {page}/{totalPages} 页 · 共 {total} 条</p>
        </>
      )}
