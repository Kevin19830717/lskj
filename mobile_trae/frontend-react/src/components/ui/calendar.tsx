...
const CalendarHeading = (props: React.HTMLAttributes<HTMLElement>) => {
  ...
  return (
    <header className="flex w-full items-center gap-1 px-1 pb-4" {...props}>
      ...
      >
        {direction === "rtl" ? <ChevronRight aria-hidden className="size-4" /> : <ChevronLeft aria-hidden className="size-4" />}
      </AriaButton>
      ...
      >
        {direction === "rtl" ? <ChevronLeft aria-hidden className="size-4" /> : <ChevronRight aria-hidden className="size-4" />}
      </AriaButton>
    </header>
  )
}
...
function JollyCalendar<T extends AriaDateValue>({ errorMessage, className, ...props }: JollyCalendarProps<T>) {
  return (
    ...
    >
      ...
      {errorMessage && <Text className="text-sm text-destructive" slot="errorMessage">{errorMessage}</Text>}
    </Calendar>
  )
}
...
function JollyRangeCalendar<T extends AriaDateValue>({ errorMessage, className, ...props }: JollyRangeCalendarProps<T>) {
  return (
    <RangeCalendar
      className={composeRenderProps(className, (className) => cn("w-fit", className))}
      {...props}
...
export {
  Calendar, CalendarCell, CalendarGrid, CalendarGridBody, CalendarGridHeader,
  CalendarHeaderCell, CalendarHeading, RangeCalendar, JollyCalendar, JollyRangeCalendar,
}
