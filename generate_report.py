...
add_para(
    '食材识别模型的训练数据集包含31类常见食材，涵盖以下类别：'
)

add_para(
    '肉类：鸡肉、猪肉、牛肉、羊肉；水产类：鱼肉、虾；蛋类：鸡蛋；豆制品：豆腐；'
    '蔬菜类：胡萝卜、西兰花、番茄、黄瓜、菠菜、土豆、蘑菇；主食类：米饭、面条、面包；'
    '水果类：苹果、香蕉、橙子；乳制品：牛奶、酸奶；坚果类：花生、核桃等，共计31个类别。'
)
...
add_para(
    '模型训练基于Ultralytics YOLO框架，以YOLO11n为基础模型进行迁移学习。训练参数设置如下：'
)

add_para(
    '输入图像尺寸：640×640；批次大小：16；初始学习率：0.01；训练轮数：100 epochs；'
    '优化器：SGD；动量：0.937；权重衰减：0.0005；预热轮数：3。'
)

add_para(
    '训练过程采用余弦退火学习率调度策略，前3轮进行线性预热，之后使用余弦衰减策略逐步降低学习率。'
    '同时使用早停（Early Stopping）机制，当验证集mAP连续15轮不再提升时停止训练，防止过拟合。'
)
...
add_para(
    '（3）输入分辨率调整：将输入图像从640×640降为320×320，进一步减少计算量。'
    '对于食材识别这种目标较大、类别较少的场景，分辨率降低对精度影响有限，但速度提升显著。'
)

add_para(
    '经过以上三步优化，最终部署在ESP32-P4上的模型大小约为900KB，推理速度达到3-5 FPS，'
...
add_para(
    '第一步：模型转换。将PyTorch训练的.pt权重文件转换为ONNX格式，然后使用ESP-DL提供的转换工具'
    '将ONNX模型转换为ESP-DL支持的模型格式。转换过程中会进行算子融合、常量折叠等图优化操作。'
)
...
add_para(
    '从测试结果来看，模型在31类食材识别任务上取得了较好的效果，满足端侧部署的精度和速度要求。'
    '对于常见食材（如米饭、鸡蛋、鸡肉等），识别准确率可达90%以上；'
    '对于外观相似的食材（如牛肉vs猪肉、不同种类的蔬菜），识别准确率相对较低，约为75-80%。'
)
...
add_para(
    '一是食材识别的精度还有提升空间。目前模型在31类食材上的mAP@0.5为0.78，对于外观相似的食材'
    '（如不同种类的肉类）区分度不够。未来可以从以下方向优化：收集更多训练数据，增加数据多样性；'
